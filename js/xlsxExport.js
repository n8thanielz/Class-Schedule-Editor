...
var _COL_WIDTHS = [20, 10, 16, 12, 14, 12, 10, 32, 14, 22, 30, 24, 14, 12, 26, 16, 14];
...
function _buildSheet(sch, fileIndex, allSchedules) {
  ...
  // Section lookup: rowIndex → first section for that row
  var byRow = {};
  ...
  // New sections grouped by course number
  var newByCourse = {};
  allSchedules.forEach(function(s2) {
    s2.sections.forEach(function(s) {
  ...
  // Map export column names → raw CSV column indices
  ...
  // Header row
  pushRow(_EXPORT_COLS, _S_HEADER);
